...
export default function Register() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // Google users may already have a profile
      navigate("/profile-setup");
    } catch (err) {
      setError(friendlyError(err.code));
    } finally {
  ...
  return (
    <div style={{ fontFamily: "Outfit, sans-serif", background: "#020817", color: "#e2e8f0", minHeight: "100vh", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, position: "relative", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 80px", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          ...
          <h1 className="fade-up d2" style={{ fontSize: "clamp(60px, 12vw, 82px)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.04em", marginBottom: 20 }}>
            Start your<br />
            <span className="shimmer-text" style={{ fontFamily: "Playfair Display, serif", fontStyle: "italic" }}>Journey</span>
          </h1>
          <p className="fade-up d3" style={{ fontSize: 15, lineHeight: 1.8, color: "rgba(226,232,240,0.5)", fontWeight: 300, maxWidth: 380 }}>
            Create your free account and get AI-powered career recommendations based on your A/L results.
          </p>
          <div className="fade-up d4" style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 36 }}>
            {["🎯 Personalised Paths", "📊 Live Market Data", "🌐 3 Languages"].map(tag => (
      ...
      {/* RIGHT PANEL */}
      <div style={{ width: 600, display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 56px", overflowY: "auto" }}>
        <div className="fade-up d1" style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(226,232,240,0.45)", fontWeight: 300 }}>
            ...
            <span onClick={() => navigate("/login")} style={{ color: "#f59e0b", cursor: "pointer", fontWeight: 600 }}>Sign in</span>
          </p>
        </div>
        ...
        <div className="fade-up d3" style={{ display: "flex", alignItems: "center", gap: 16, margin: "20px 0" }}>
        ...
        {error && (
          <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 6, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#fca5a5" }}>
            ⚠️ {error}
          </div>
        )}
        ...
        <form onSubmit={handleRegister} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="fade-up d3">
            <label style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(226,232,240,0.5)", display: "block", marginBottom: 8 }}>Full Name</label>
            <input className="input-field" placeholder="e.g. Kasun Perera" value={fullName} onChange={e => setFullName(e.target.value)} required />
          </div>
          ...
          <div className="fade-up d4">
            ...
            <div style={{ position: "relative" }}>
              <input type={showPass ? "text" : "password"} className="input-field" placeholder="Min. 6 characters" value={password} onChange={e => setPassword(e.target.value)} style={{ paddingRight: 52 }} required />
              <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "rgba(226,232,240,0.4)", fontSize: 13, fontFamily: "Outfit, sans-serif", fontWeight: 500 }}>
                {showPass ? "hide" : "show"}
              </button>
            </div>
          </div>
          ...
          <div className="fade-up d4">
            <label style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(226,232,240,0.5)", display: "block", marginBottom: 8 }}>Confirm Password</label>
            <input type={showPass ? "text" : "password"} className="input-field" placeholder="Re-enter password" value={confirm} onChange={e => setConfirm(e.target.value)} required />
          </div>

          <div className="fade-up d5" style={{ marginTop: 6 }}>
        ...
        <p className="fade-up d5" style={{ marginTop: 24, fontSize: 12, color: "rgba(226,232,240,0.25)", textAlign: "center", lineHeight: 1.7 }}>
